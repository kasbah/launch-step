    ...
    function reducer(state, action) {
        ...
        switch (action.type) {
            ...
            case 'set-button-timer':
                {
                    var _x = action.value[0];
                    var _y = action.value[1];
                    clearInterval(state.buttonTimerGrid[_x][_y]);
                    state.buttonTimerGrid[_x][_y] = action.value[2];
                    return state;
                }
            case 'toggle-button':
                {
                    var _x2 = action.value[0];
                    var _y2 = action.value[1];
                    if (state.stepGrid[_x2][_y2] == state.velocityBrush) {
                        state.stepGrid[_x2][_y2] = 0;
                    } else {
                        state.stepGrid[_x2][_y2] = state.velocityBrush;
                    }
                    return state;
                }
            case 'set-playing':
                {
                    var _y3 = action.value;
                    state.playing[_y3] = true;
                    return state;
                }
            case 'set-off':
                {
                    var _y4 = action.value;
                    state.playing[_y4] = false;
                    return state;
                }
            case 'set-step':
                {
